...
%title
\edef\mytitle{Essence of Precalculus}
\edef\mysubtitle{Lesson 1: Math Fundamentals}
...
\tableofcontents
\section{Set Theory}
Often in mathematics, we like to group abstract things, also known as mathematical objects, into sets. A mathematical object is, simply put, a \textit{thing} we like to talk about in math, such as functions, numbers, matrices, other sets, etc.
Although seemingly simple, these sets are incredibly powerful, so powerful in fact that most modern mathematics today is built on the backbones of set theory.
...
Simply put, given any set of sets that are not empty, one can always find a way to map a child set to an element in that child set. Though seemingly simple, due to its complications with infinite sets, it has deep consequences in all mathematics and how we deal with infinities in the universe.}
\subsection{Notation}
Before we move any further, we should get some basic notation out of the way.
Below is a list of all the important symbols that we will be using in this class:
\begin{center}
...
Although both would have been equally valid, I specifically chose the former to shed light on the fact that the two sets $B$ and $A$ are not equal.
...
In particular, when we say two sets are equal, we should expect that every element in one set should be in the other and vice versa. In other words, we expect
...
I emphasize the "and" and "or" parts of the previous statements since I believe it's important to see the relationship between these very mathematical ideas and the words we use in our everyday lives.\footnote{
...
Now, with this intuition of these operations, we can move on to rigorously define them as follows:
\begin{define}
...
\end{define}
\footnotetext{Notice, we could have also defined $\bar A$ as $U\setminus A$, which has the advantage of specifying a global set over which we are taking the complement over.}
...
where $\real^2$ is the set of all real ordered pairs.
...
$$S=(2,5).$$
Alternatively, we can change the parentheses to square brackets, where we would then have
$$S=[2,5]=\{x : 2 \le x \le 5\}.$$
...
Certain sets in mathematics are so important that we give them a special symbols.
\begin{center}
	\begin{longtable}{| c | m{8cm} |}
		\hline
		Symbol & Definition \\
...
This might seem at first like a bunch of mathematical mumble jumbo, but it has serious consequences when it comes to mathematical reasoning.
...
Then, looking further, we realize by taking any subset of the natural numbers, finite or infinite, as long as this set isn't empty, there must be a least element since no matter what is in any of these subsets, 1 is always less than everything, and if 1 isn't in the set, we go to 2 or 3, and the same logic applies. This is a well-ordered set; we take this as a defining property of natural numbers.\footnote{Rigorously proving this fact $\nat$, using the formal definition of $\nat$ is very difficult and confusing.}
...
which is exactly the expression we would get if we substituted $m+1$ into our formula. Therefore, we have proven that if the formula is true for $m$, then it must be true for $m+1$. Let's call this part the inductive step.
...
Philosophically speaking, in general, in everyday life, inductive reasoning is inherently flawed since your conclusions rely on possibly flawed assumptions about the past.
...
We can make another example of this idea: suppose we define a statement $Q(n)$ as \textbf{$n$ is odd}. Then we can define a set $S=\{Q(n):n\in\nat\}$. Then we know $Q(1),Q(3),Q(5),...$ is true and $Q(2),Q(4),Q(6),...$ is obviously false. Now equipped with this, we will attempt to prove our first lemma:
\begin{lemma}
...
\begin{proof}
	...
	It follows that $F\subseteq S$.
	By our assumption, $P(k)\in F$ hence $F\neq \varnothing$.
	...
	However, this means $P(n_l)$ must also be true by the inductive step assumption, thus a contradiction of our original assumption.
	Therefore, $F\equiv \varnothing$.
...
\begin{ex}
	...
	To prove the previous statement, we leverage induction again. By substituting $k=1$, we find the base case is trivial. Then, for our inductive step, we let $m\in\nat$ such that $2^{m+1}>2m+1$.
	Then
