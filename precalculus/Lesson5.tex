...
\begin{enumerate}
	...
	\item Retain every property as outlined in proposition \eqref{prop:exponent-prop}.
...
\begin{proof}
	...
	Thus we have $\phi(-x)=\frac{1}{\phi(x)}$ by algebric manipulation, thereby proving statement \eqref{prop:exp-prop-neg}.
	...
	For statment \eqref{prop:exp-prop-non-zero}, suppose there exists $x \in \real$ such that $\phi(x)=0$.
	Observe
...
\begin{prop} \label{prop:basic-root-prop}
	...
	\begin{enumerate}
		\item $a^n$ is an strictly increasing function if $a>1$
		\item $a^n$ is an strictly decreasing function if $a<1$.
...
\begin{prop} \label{prop:exp-Q-prop}
	$[a]_\rat$ as defined in proposition \eqref{prop:exp-Q-ext} satifies the following:
	\begin{enumerate}
		...
		\item $[a]_\rat$ is strictly increasing for $a>1$, and stictly decreasing for $a<1$. \label{prop:exp-Q-inc-prop}
...
\begin{proof}
	...
	$[a]_\rat$ follows immediatly by the definition of $n$-powers and $n$-roots ($n$-roots are always defined to be postive). 
	
	Suppose $a>1$.
	If $r < s$, then $\frac{m}{n}<\frac{p}{q}$.
	Since we assumed $n,q>0$, we have $mq < np$.
	By proposition \eqref{prop:basic-root-prop}, we have $a^{mq} < a^{np}$, since for $a>1$, $a^k$ is an strictly increasing function for integers.
	...
	$\sqrt[n] {a^m} < \sqrt[q] {a^p}$ is equivilent to $[a]\paren{\frac{m}{n}} < [a] \paren{\frac{p}{q}}$.
...
\end{prop}
\begin{proof}
	$[a]$ is clearly continuous and $[a](1)=a$ follows immediatly from the definition.
...
\begin{prop} \label{prop:exp-extend-properties}
	...
	\begin{enumerate}
		\item $[a]$ is stictly increasing if $a>1$, stictly decreasing if $a<1$ and constant if $a=1$.
...
\begin{prop} \label{prop:exp-range}
	For any $a$, the exponetial with base $a$ has the range $(0,\infty)$.
\end{prop}
\begin{proof}
	...
	Since $\frac{1}{N}$ is postive, we have $\frac{1}{a^{-x}} = a^x < N$.
...
I will leave the analytical proof of this fact as an exercise for our readers.\footnote{
	...
	The fact that $a^x$ is an open map follows immediately from the fact the fact $a^x$ is strictly increasing (decreasing) and by the intermediate value theorem.
...
Then to find the image, immediatly from the definition \eqref{def:trigdefine}, $|\sin(x)|,|\cos(x)|\le 1$.
...
Since $\csc(x)$ and $\text{sec}(x)$ are just the reciprocals of $\sin(x)$ and $\cos(x)$ respectivly, immediatly we get the range of these functions is the compound interval $[-\infty,-1]\cup[1,\infty]$.
...
\begin{proof}
	...
	If $\theta \in [-\pi,\pi]$, negating $\theta$ corisponds to a reflection of over the $x$-axis.
...
\begin{proof}
	...
	By subsitution into equation \eqref{eq:half-angle-eq1}
	\begin{align*}
...
In figure \eqref{fig:trig-estimate-plot}, given that $\odot \, O$ is the unit circle, then the lenght of $\overline {AC}$ is $\sin\theta$.
...
\end{cor}
\begin{proof}
	Immediate from the definition of these functions.
\end{proof}
\end{document}
